  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="核销时间" min-width="150px">
        <template slot-scope="{row}">
          <span v-if="row.confirmTime ">{{ row.confirmTime | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="回收类别" min-width="250px">
        <template slot-scope="{row}">
          <span>{{ row.recycleType }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="回收费用" min-width="150px">
        <template slot-scope="{row}">
          <el-tag>{{ row.fee }}</el-tag>
        </template>
      </el-table-column>
...
<script>
import { fetchList } from '@/api/doorRecycle'
